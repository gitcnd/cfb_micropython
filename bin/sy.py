...
# 1. From REPL shell
#    >>> ARGV=["pic.jpg"];exec(open("bin/sy.py").read())
...
def __main__(args):
    sy(args[2:])    # mipyshell first 2 arguments are "python" and "photo.py"



# This is a stripped down micropython version.
# getc is not working yet (terminal emulator issue?)

# From https://github.com/danielkucera/xmodem
# Use  sys.stdin.buffer.read(1) and sys.stdout.buffer.write(b'x')
...
# Works(linux)!  python -u -c 'import ymodem;ymodem.sy("out")'
...
class XMODEM(object):
    ...
    def __init__(self, getc, putc, mode='ymodem', pad=b'\x1a', log=None):
        self._getc = getc
        self.putc = putc
        self.mode = mode
        self.pad = pad
    ...
    def send(self, stream, retry=16, timeout=60, quiet=False, callback=None):
        ...
        cancel = 0
        while True:
            self.log('debug','calling getc')
            char = self.getc(1, timeout)
            self.log('debug','got:%s' % char)
            if char:
                if char == NAK:
                    ...
                    self.log('error','send error: expected NAK, CRC, or CAN; got %r %s %s', char,CRC,char)
        ...
        while True:
            ...
            if self.mode == 'ymodem' and success_count == 0:
                ...
                if len(filenames):
                    ...
                    data = filename.encode() + NUL + str(stat.st_size).encode()
            ...
            while True:
                ...
                if char == ACK:
                    ...
                    if self.mode == 'ymodem' and success_count == 1 and len(filename):
                        ...
                        if char == CRC:
                            break
                        self.log('error','send error: ymodem expected CRC; got %r for block %d', char, sequence)
                    else:
                        break

                self.log('error','send error: expected ACK; got %r for block %d', char, sequence)
                error_count += 1
                if callable(callback):
                    callback(total_packets, success_count, error_count)
                if error_count > retry:
    ...
    def recv(self, stream, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0):
        ...
        while True:
            ...
            if not (seq1 == seq2 == sequence):
                ...
                self.log('error','expected sequence %d, got (seq1=%r, seq2=%r), receiving next block, will NAK.', sequence, seq1, seq2)
...
#try:
#finally:
#termios.tcsetattr(sys.stdin, termios.TCSADRAIN, original_settings) # Restore the original terminal settings


def log(loglevel,msg,*args):
...
def stdgetc(size, timeout=1):
    log('debug','in getc for %d' % size);
    #return 'C'
    return sys.stdin.buffer.read(size) or None

def stdputc(data, timeout=1):
    log('debug','in putch with %s' % data.hex());
    sys.stdout.buffer.write(data) # ser.write(data)  # note that this ignores the timeout
    #os.fsync(sys.stdout)
    #return sys.stdout.write(data) # ser.write(data)  # note that this ignores the timeout - mangles [255]

def sz():
    # from xmodem import XMODEM
    #modem = XMODEM(stdgetc, stdputc, mode='ymodem', log=log)
    modem = XMODEM(stdgetc, stdputc, mode='ymodem')
    stream = open('testx.txt', 'rb')
    print(modem.send(stream))

def sx(fn):
    log('debug','testing sx on file %s',fn);
    #original_settings = termios.tcgetattr(sys.stdin) # Save the original terminal settings
    #tty.setraw(sys.stdin)
    #tty.setraw(sys.stdout)
    #modem = XMODEM(stdgetc, stdputc, mode='xmodem', log=log)
    modem = XMODEM(stdgetc, stdputc, mode='xmodem') # no logs please
    #stream = open('testx.txt', 'rb')
    stream = open(fn, 'rb')
    #log('debug','see above')
    print(modem.send(stream))
    #modem = XMODEM(stdgetc, stdputc, mode='xmodem')
    #modem.send(['/boot.py'])
    #tty.setcbreak(sys.stdin)

def sy(fn):
    log('debug','testing sy on file %s',fn);
    #original_settings = termios.tcgetattr(sys.stdin) # Save the original terminal settings
    #tty.setraw(sys.stdin)
    #tty.setraw(sys.stdout)
    modem = XMODEM(stdgetc, stdputc, mode='ymodem')
    #modem.send(['testx.txt'])
    modem.send([fn])
    #tty.setcbreak(sys.stdin)
